      ...
      getChangedRanges(transform).forEach(({ newRange }) => {
        ...
        if (textBlock && textBeforeWhitespace) {
          ...
          linksBeforeSpace.filter((link) => link.isLink).map((link) => ({
            ...
            if (getMarksBetween(link.from, link.to, newState.doc).some((item) => item.mark.type === options2.type)) return;
            tr2.addMark(link.from, link.to, options2.type.create({ href: link.href }));
          });
        }
      });
...
function clickHandler(options2) {
  return new Plugin({
    ...
    props: { handleClick: (view, pos, event) => {
      ...
      let handled = false;
      if (options2.enableClickSelection) handled = options2.editor.commands.extendMarkRange(options2.type.name);
...
function pasteHandler(options2) {
  return new Plugin({
    ...
    props: { handlePaste: (view, _event, slice) => {
      ...
      return options2.editor.commands.setMark(options2.type, { href: link.href });
    } }
  });
}
...
//# sourceMappingURL=chunk-6JF62Q3R.js.map
