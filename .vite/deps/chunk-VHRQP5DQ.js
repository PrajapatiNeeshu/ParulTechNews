...
function createNodeFromContent(content, schema, options) {
  ...
  if (isTextContent) {
    if (options.errorOnInvalidContent) {
      ...
      if (options.errorOnInvalidContent && hasInvalidContent) throw new Error("[tiptap error]: Invalid HTML content", { cause: new Error(`Invalid element found: ${invalidContent}`) });
    }
...
var insertContentAt = (position, value, options) => ({ tr, dispatch, editor }) => {
  if (dispatch) {
    ...
    if (options.updateSelection) selectionToInsertionEnd2(tr, tr.steps.length - 1, -1);
...
var insertDefaultBlock = (options = {}) => ({ tr, dispatch, editor }) => {
  ...
  if (dispatch) {
    ...
    if (updateSelection2) selectionToInsertionEnd2(tr, tr.steps.length - 1, -1);
  }
  return true;
};
...
var setContent = (content, { errorOnInvalidContent, emitUpdate = true, parseOptions = {} } = {}) => ({ editor, tr, dispatch, commands }) => {
  ...
  if (parseOptions.preserveWhitespace !== "full") {
    ...
    if (dispatch) {
      ...
      tr.replaceWith(0, doc3.content.size, nodes).setMeta("preventUpdate", !emitUpdate);
    }
    return true;
  }
...
function generateHTML(doc3, extensions) {
  ...
  return getHTMLFromFragment(Node.fromJSON(schema, doc3).content, schema);
}
...
function generateText(doc3, extensions, options) {
  const { blockSeparator = "\n\n", textSerializers = {} } = options || {};
  const schema = getSchema(extensions);
  return getText2(Node.fromJSON(schema, doc3), {
    blockSeparator,
    textSerializers: {
...
function getMarksBetween(from, to, doc3) {
  const marks = [];
  if (from === to) doc3.resolve(from).marks().forEach((mark) => {
    const range = getMarkRange(doc3.resolve(from), mark.type);
    if (!range) return;
    marks.push({
      mark,
      ...range
  ...
  else doc3.nodesBetween(from, to, (node, pos) => {
...
function isNodeViewSelected({ selection, pos, nodeSize: nodeSize2, selectedOnTextSelection = false }) {
  const { from, to } = selection;
  if (from <= pos && to >= pos + nodeSize2) return true;
  if (selectedOnTextSelection && isTextSelection(selection) && from > pos && to < pos + nodeSize2) return true;
  return false;
}
...
var commands_exports = __exportAll({
  blur: () => blur,
...
function getRebuildRanges(tr, doc3) {
  ...
  */
  ...
  static handleExit({ editor, mark }) {
    ...
    if (currentPos.pos === currentPos.end()) {
      ...
      if (removeMark2) tr.removeStoredMark(removeMark2);
  ...
  addProseMirrorPlugins() {
    return [new Plugin({
      ...
      appendTransaction: (transactions, oldState, newState) => {
        ...
        if (empty2 || !(from === allFrom && to === allEnd)) return;
...
var extensions_exports = __exportAll({
...
var Decoration2 = class {
  ...
  */
  static Widget(pos, render2, options) {
    const { key, ...spec } = options;
    return new WidgetDecoration(pos, render2, key, spec);
  }
};
var InlineDecoration = class extends Decoration2 {
  ...
  toPMDecoration(extensionName) {
    ...
    return Decoration.inline(this.from, this.to, this.attrs, spec);
  }
};
var NodeDecoration = class extends Decoration2 {
  ...
  toPMDecoration(extensionName) {
    ...
    return Decoration.node(this.from, this.to, this.attrs, spec);
  }
};
var WidgetDecoration = class extends Decoration2 {
  constructor(pos, render2, key, spec) {
    super();
    this.kind = "widget";
    this.pos = pos;
    this.render = render2;
  ...
  toPMDecoration(extensionName) {
    ...
    return Decoration.widget(this.pos, this.render, spec);
  }
};
...
function createWidgetDecoration(options) {
  ...
  return Decoration2.Widget(pos, render2, {
    key,
    side,
    relaxedSide,
    marks,
...
//# sourceMappingURL=chunk-VHRQP5DQ.js.map
